...
public class Batgioi extends Npc {

    public Batgioi(int mapId, int status, int cx, int cy, int tempId, int avartar) {
    ...
    public void openBaseMenu(Player player) {
        if (canOpenNpc(player)) {
            switch (mapId) {
                case 122 -> {
                    createOtherMenu(player, ConstNpc.BASE_MENU, "Việc khó quá có ATM lo!!!\n", "Cửa hàng"
...
//                            player.pointtet+=1;
                        ...
                        if (select == 0) {
                            ShopService.gI().opendShop(player, "NGO_KHONG", true);
                        }
                        if (select == 1) {
